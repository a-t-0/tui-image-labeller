...
class AddressSelector:
    def __init__(self):
        ...
        self.selected_shop = None  # Track selected shop
        ...
        self.palette = [
            ...
            ("history_suggestions", "light green", "black"),  # Added style
        ]
    ...
    def build_select_widget(self):
        ...
        for idx, shop in enumerate(self.shops[start_idx:end_idx], 1):
            ...
            # Apply highlight if this shop is selected
            if self.selected_shop and self.selected_shop == shop:
                attr_border = urwid.AttrMap(border, "history_suggestions")
            else:
        ...
        if self.mode == "select":
            if key in ("1", "2", "3", "4", "5"):
                ...
                if start_idx + selection < len(self.shops):
                    self.selected_shop = self.shops[start_idx + selection]
                    print(f"Selected: {self.selected_shop.to_string()}")
                    self.main_widget = (
                        self.build_select_widget()
                    )  # Rebuild to show highlight
                    self.loop.widget = self.main_widget
                    # Do not exit loop to allow further interaction
                    # raise urwid.ExitMainLoop()
                ...
                self.mode = "manual"
                self.selected_shop = None  # Clear selection in manual mode
            ...
            if key == "enter":
                ...
                if self.current_question_idx >= len(self.questions):
                    ...
                    print(f"Manual entry: {self.selected_shop.to_string()}")
                    self.mode = "select"
                    self.current_batch = 0
                    self.manual_answers = []
                    self.current_question_idx = 0
